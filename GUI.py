...
class JeopardyGUI:
    ...
    def show_welcome_screen(self):
        ...
        self.window.after(100, lambda: animate_letter("Welcome to", screen_height * 0.2, 0))
        self.window.after(800, lambda: animate_letter("Meow-pardy! 😺", screen_height * 0.3, 0))
        self.window.after(1500, lambda: animate_letter("Can you beat the A.I.? 🤖", screen_height * 0.4, 0))
        self.window.after(2500, self.add_paw)

        # Animate welcome text
        def animate_letter(text, y_pos, delay_start, letters=None):
            if letters is None:
                letters = []
            ...
            for i, char in enumerate(text):
                ...
                animate_size(letter, i, delay_start + (i * 100))
    ...
    def add_paw(self):
        ...
        def on_leave(e):
            ...
            # Reset paw size
            self.paw_button.configure(image=self.paw_photo)
        ...
        def glow_animation():
            ...
            def cycle_colors(index=0):
                if index < len(colors):
                    ...
                    self.glow_after_id = self.window.after(100, lambda: cycle_colors(0))
        ...
        def initial_pulse(size=1.0, growing=True):
            if not hasattr(self, 'pulse_count'):
                self.pulse_count = 0

            if self.pulse_count < 6:  # Do 3 full pulses
                ...
                self.window.after(50, lambda: initial_pulse(size, growing))

        self.paw_button.pack()
        initial_pulse()  # Start the initial pulse animation
